...
//
#define VCP_RX_BUFF_SIZE 128
static unsigned char  paucRxBuffer[VCP_RX_BUFF_SIZE];
static RingBuff srRingBuffRx;
//
...
static uint16_t VCP_Init(void)
{	//
	setupRingBuff(&srRingBuffRx, paucRxBuffer, VCP_RX_BUFF_SIZE, NULL);
	//
	return USBD_OK;
}
...
/*
	...
	for (i = 0; i < Len; i++)
	{
        ...
        if(APP_tx_ptr_head == APP_TX_BUF_SIZE)
        	APP_tx_ptr_head = 0;
        // check if over write is happened or not.
        if(APP_tx_ptr_head == APP_tx_ptr_tail)
        	return USBD_FAIL;
	}
*/
	setToRingBuff(&srRingBuffRx, Buf, Len);
    //
	return USBD_OK;
}
...
{
	uint8_t cnt;
	cnt = getFromRingBuff(&srRingBuffRx, buf, 1);
	return cnt;
/*
	if(APP_tx_ptr_head == APP_tx_ptr_tail)
		return 0;
...
{
	uint8_t cnt;
	cnt = getFromRingBuff(&srRingBuffRx, buf, VCP_RX_BUFF_SIZE);
	return cnt;
/*
	if(APP_tx_ptr_head == APP_tx_ptr_tail)
		return 0;
